...
import { fetchFavouriteColleges } from '../../utils/api';
...
const Navbar = () => {
  ...
  useEffect(() => {
    fetchFavouriteColleges().then(setFavColleges).catch(() => setFavColleges([]));
  }, []);
          ...
          <div className="flex items-center gap-1">
            ...
            >
              ...
              {favColleges.length > 0 ? (
                favColleges.map((college) => (
                  ...
                  >
                    <span className="mr-2">{college.icon || '🏛️'}</span>
                    {college.short_name}
                  </Dropdown.Item>
                ))
              ) : (
                <Dropdown.Item disabled>No featured colleges</Dropdown.Item>
              )}
              <Dropdown.Divider />
            ...
            {isAuthenticated ? (
              ...
              >
                ...
                <Dropdown.Divider />
                <Dropdown.Item danger onClick={handleLogout}>
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="px-3 py-2 text-sm font-medium text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors"
                >
                  Sign In
                </Link>
                <Link
